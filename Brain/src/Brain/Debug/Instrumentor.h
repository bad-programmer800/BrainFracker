	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					BR_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					BR_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
...
#define BR_PROFILE 0
	...
	#define BR_PROFILE_BEGIN_SESSION(name, filepath) ::Brain::Instrumentor::Get().BeginSession(name, filepath)
	#define BR_PROFILE_END_SESSION() ::Brain::Instrumentor::Get().EndSession()
	#define BR_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = 
					::Brain::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
